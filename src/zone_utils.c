...
void zone_print_mask(t_zone *zone)
{
	...
	while (i < 8 * zone_bytes_size(zone->size))
	{
		(last_bytes[i / 32] >> i % 32) & 1 ? write(1, "1", 1) : write(1, "0", 1);
		i++;
	}
	write(0, "\n", 1);
}
							   ...
							   uint32_t *last_bytes, size_t i)
{
	...
	first_byte = 0;
	while (((first_bytes[i / 32] >> (i % 32)) & 1L) == 0 && i < current_size)
		i++;
	first_byte = i;
	while (((last_bytes[i / 32] >> (i % 32)) & 1L) == 0 && i < current_size)
		i++;
	if (i >= current_size)
		return (current_size);
...
void zone_print(t_zone *zone)
{
	...
	if (zone->id == zone_empty_id)
		return;
	if (zone->id == zone_tiny_id)
		write(1, "TINY : ", 7);
	else if (zone->id == zone_small_id)
		write(1, "SMALL : ", 8);
	else
		write(1, "LARGE : ", 8);
	write_ptr(zone->data);
